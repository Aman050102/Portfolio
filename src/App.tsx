        ...
        <section id="projects" className="py-16">
          <div className="mb-10 flex items-center gap-4">
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white rounded-xl border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(251,191,36,1)]">
              <Terminal size={18} />
              <h2 className="text-xs font-black uppercase tracking-wider">Featured QA Automation Work 🛠️</h2>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8 bg-[#FFFDF6] p-6 rounded-[2rem] border-4 border-slate-800 shadow-[8px_8px_0px_0px_rgba(30,41,59,1)] mb-12">
              <div className="space-y-5 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-slate-800 tracking-tight">{upRegProject.title}</h3>
                  <p className="text-slate-600 font-medium leading-relaxed mt-3">{upRegProject.description}</p>
                </div>

                <div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {upRegProject.tags.map(t => (
                      <span key={t} className="text-xs font-bold text-slate-500">#{t}</span>
                    ))}
                  </div>
                  <a href={upRegProject.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-slate-800 text-white px-5 py-3 rounded-xl font-black text-sm border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(30,41,59,1)] hover:translate-y-[-2px] transition-all">
              ...
              <div className="bg-slate-900 border-4 border-slate-800 rounded-2xl p-5 text-xs font-mono text-amber-200 shadow-[6px_6px_0px_0px_rgba(30,41,59,1)] relative overflow-hidden">
                <div className="flex gap-1.5 mb-4 border-b border-slate-700 pb-3">
                  ...
                  <span className="text-[10px] text-slate-400 ml-2">gpa_calculate.robot</span>
                </div>
                <div className="space-y-1">
                  <p><span className="text-pink-400">*** Test Cases ***</span></p>
                  <p className="text-sky-300">TC-01: Maximum GPA (All A)</p>
                  <p className="pl-4 text-slate-400">Input Multiple Grades  A  A  A</p>
                  <p className="pl-4 text-emerald-400">Wait Until Page Contains  4.00</p>
                  <br />
                  <p className="text-sky-300">TC-03: Minimum GPA (All F)</p>
                  <p className="pl-4 text-slate-400">Input Multiple Grades  F  F  F</p>
                  <p className="pl-4 text-emerald-400">Wait Until Page Contains  0.00</p>
                </div>
              </div>
            </div>
          )}
